/// \file
/// \brief odometry_slam node - which publishes odometry messages and the odometry transform (for the blue robot)
///
...
///     path (nav_msgs/Path): publish the path of the odometry robot
/// SUBSCRIBES:
...
static std::string odom_id = "world";
...
int main(int argc, char** argv){
    ...
    ROS_INFO("odometry_slam node in the loop.");
    ros::init(argc,argv,"odometry_slam");
    ros::NodeHandle nh;
    param_initialization(nh);

    path_pub = nh.advertise<nav_msgs::Path>("path", 1000);
    Path.header.frame_id = odom_id;
    PoseStamped.header.frame_id = odom_id;
    ...
    while(ros::ok()){
        ...
        ros::spinOnce();
        loop_rate.sleep();
        //ROS_INFO("[x,y,theta]:",x_0,y_0,theta_0);
    }

    ROS_INFO("odometry_slam node is closed.");
    return 0;
...
bool broadcast_transform(tf2_ros::TransformBroadcaster& odom_broadcaster){
    ...
    //ROS_INFO("[x,y,theta]1: %f,%f,%f",transform.translation().x,transform.translation().y,transform.rotation());
