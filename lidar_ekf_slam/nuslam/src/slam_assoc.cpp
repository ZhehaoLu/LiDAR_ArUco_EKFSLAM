/// \file
/// \brief slam_assoc node - which visualizes the slam robot (both map frame and
/// odom frame) with EKF algorithm (unknown data association)
///
...
///     id_to_statevec:  mapping from landmark_id to the index of state vector
///     (epsilon)
/// PUBLISHES:
///     odom (sensor_msgs/JointState):  publishes odometry messages
///     pose (turtlesim/Pose): publish the actual pose
///     slam_marker (visualization_msgs/MarkerArray): publish the marker postion
///     detected by slam robot
/// SUBSCRIBES:
///     joint_states (sensor_msgs/JointState): subscribe to joint_states (to
///     calculate odometry) sensed_landmark (visualization_msgs/MarkerArray):
///     get measurements (from laser scan data)
...
Eigen::Matrix<double, 3, 3> Q =
    (Eigen::Matrix<double, 3, 3>() << 0.001, 0, 0, 0, 0.001, 0, 0, 0, 0.001)
        .finished();
Eigen::Matrix<double, 2, 2> R =
    (Eigen::Matrix<double, 2, 2>() << 0.01, 0, 0, 0.01).finished();
...
public:
  measure(int id, double range, double bear)
      : m_id(id), m_range(range), m_bear(bear){};
  ~measure(){};
};
...
bool broadcast_transform_maptoodom(
    tf2_ros::TransformBroadcaster &odom_broadcaster);

/// \brief set the pose of the robot from srv
/// \return - true or false
// bool set_pose_callback(nuturtle_control::set_pose::Request &req,
// nuturtle_control::set_pose::Response &res);
...
/// \brief calculate and store the global position of measurement from
/// markerarray data
...
/// \brief update state vector (epsilon) by EKF algorithm (known data
/// association)
void EKF_update(turtlelib::Transform2D &transform, turtlelib::Twist2D &t);

/// \brief pubblish odom and pose
void odom_pose_publish(turtlelib::Transform2D &transform,
                       turtlelib::Twist2D &t);
...
int main(int argc, char **argv) {
  ...
  diff_drive_slam = turtlelib::DiffDrive(
      turtlelib::Transform2D(turtlelib::Vector2D(x_0, y_0), theta_0),
  ...
  sense_landmark_sub =
      nh.subscribe("/sensed_landmark", 1000, sense_landmark_callback);

  // fake_sensor_sub = nh.subscribe("/fake_sensor", 1000, fake_sensor_callback);
  slam_marker_pub =
      nh.advertise<visualization_msgs::MarkerArray>("/slam_marker", 10);

  // sim_lidar_sub = nh.subscribe("/sim_lidar",1000,sim_lidar_callback);
  // slam_marker_pub =
  // nh.advertise<visualization_msgs::MarkerArray>("slam_marker",50);

  // ros::ServiceServer set_pose_service = nh.advertiseService("set_pose",
  // set_pose_callback);
  tf2_ros::TransformBroadcaster odom_broadcaster;
...
void sense_landmark_callback(const visualization_msgs::MarkerArray &msg) {
  ...
  // {
  //     cout << "pos:" << msg.markers[i].pose.position.x << "," <<
  //     msg.markers[i].pose.position.y << endl;
  // }
...
void EKF_update(turtlelib::Transform2D &transform, turtlelib::Twist2D &t) {
  ...
  if (turtlelib::almost_equal(t.m_angular_v, 0.0)) {
    ...
    At(1, 0) = -(t.m_translation_vx / t.m_angular_v) * std::cos(epsilon(0)) +
               (t.m_translation_vy / t.m_angular_v) *
                   std::cos(epsilon(0) + t.m_angular_v);
    At(2, 0) = -(t.m_translation_vx / t.m_angular_v) * std::sin(epsilon(0)) +
               (t.m_translation_vy / t.m_angular_v) *
                   std::sin(epsilon(0) + t.m_angular_v);
  }

  Eigen::MatrixXd Q_extended =
      Eigen::MatrixXd::Zero(sigma.rows(), sigma.cols());
  ...
  if (if_mea) {
    ...
    for (auto const &m : mea) {
      ...
      for (int i = 0; i < landmark_cnt; i++) {
        ...
        double range_calculate =
            std::sqrt(std::pow(epsilon(idx) - epsilon(1), 2) +
                      std::pow(epsilon(idx + 1) - epsilon(2), 2));

        double bear_calculate =
            turtlelib::normalize_angle(std::atan2(epsilon(idx + 1) - epsilon(2),
                                                  epsilon(idx) - epsilon(1)) -
                                       epsilon(0));
        ...
        // std::cout<<"["<<" "<<i<<"]: "<<" zi:"<<zi(0)<<" "<<zi(1)<<"
        // zk:"<<zk(0)<<" "<<zk(1)<<std::endl; std::cout<<"
        // z_diff:"<<z_diff(0)<<" "<<z_diff(1)<<" dist:"<<dist<<std::endl;
      ...
      if (dist_min == dist_threshold) {
        ...
        epsilon(idx + 1) =
            epsilon(2) + m.m_range * std::sin(m.m_bear + epsilon(0));
      ...
      double range_calculate =
          std::sqrt(std::pow(epsilon(assoc_idx) - epsilon(1), 2) +
                    std::pow(epsilon(assoc_idx + 1) - epsilon(2), 2));
      double bear_calculate = turtlelib::normalize_angle(
          std::atan2(epsilon(assoc_idx + 1) - epsilon(2),
                     epsilon(assoc_idx) - epsilon(1)) -
          epsilon(0));
      ...
      Eigen::MatrixXd K =
          sigma * H.transpose() * ((H * sigma * H.transpose() + R).inverse());
      ...
      sigma =
          (Eigen::MatrixXd::Identity(epsilon.size(), epsilon.size()) - K * H) *
          sigma;
    }
    if_mea = false;
  }
...
void odom_pose_publish(turtlelib::Transform2D &transform,
  ...
  odom.pose.pose.orientation =
      tf::createQuaternionMsgFromYaw(transform.rotation());
...
void joint_state_callback(const sensor_msgs::JointState::ConstPtr &msg) {
  ...
  // ROS_INFO("%f, %f, %f :", transform.translation().x,
  // transform.translation().y, transform.rotation());
  ...
  diff_drive_slam.setpose(turtlelib::Transform2D(
      turtlelib::Vector2D(epsilon(1), epsilon(2)), epsilon(0)));
...
bool broadcast_transform(tf2_ros::TransformBroadcaster &odom_broadcaster) {
  ...
  // ROS_INFO("[x,y,theta]:
  // %f,%f,%f",transform.translation().x,transform.translation().y,transform.rotation());
...
bool broadcast_transform_maptoodom(
  ...
  // turtlelib::Transform2D transform = diff_drive.getpose();
  turtlelib::Transform2D T_m_b = turtlelib::Transform2D(
      turtlelib::Vector2D(epsilon(1), epsilon(2)), epsilon(0));
  turtlelib::Transform2D T_o_b = turtlelib::Transform2D(
      turtlelib::Vector2D(transform.translation().x, transform.translation().y),
      transform.rotation());
  turtlelib::Transform2D T_m_o = T_m_b * (T_o_b.inv());

  geometry_msgs::Quaternion q =
      tf::createQuaternionMsgFromYaw(T_m_o.rotation());
  ...
  geometry_msgs::Quaternion goal_quat =
      tf::createQuaternionMsgFromYaw(T_m_b.rotation());
