...
/// \file
/// \brief turtle interface node - which implements control of the turtlebot via geometry_msgs/Twist message
///
...
/// 
/// PUBLISHES:
///     joint_state (sensor_msgs/JointState):  provide the angle (in radians) and velocity (in rad/sec)
///     wheel_cmd (nuturtlebot/WheelCommands): make the turtlebot follow the specified twist
/// SUBSCRIBES:
...
public:
    ...
    /// \brief transform sensor data (encoder readings in the range of [0,4096]) into joint states
    void sensor_data_callback(const nuturtlebot_msgs::SensorData &msg);

    /// \brief transform twist into wheelcommands ([-256,256] that are proportional to wheel velocity)
...
Interface::Interface(ros::NodeHandle &nh)
{
    ...
    // param_test();
    diff_drive = turtlelib::DiffDrive(turtlelib::Transform2D(turtlelib::Vector2D(0.0, 0.0), 0.0), m_track_width, m_wheel_radius);
    // ROS_INFO("%f,%f:", m_track_width,m_wheel_radius);
    sensor_data_sub = nh.subscribe("/sensor_data", 1000, &Interface::sensor_data_callback, this);
    cmd_vel_sub = nh.subscribe("cmd_vel", 1000, &Interface::cmd_vel_callback, this);
...
void Interface::param_initialization(ros::NodeHandle &nh)
{
    ...
    m_wheel_linear_maxvel = m_motor_cmd_max * m_motor_cmd_to_radsec * m_wheel_radius;
...
void Interface::sensor_data_callback(const nuturtlebot_msgs::SensorData &msg)
{
    ...
    turtlelib::WheelVel vel_calculate = diff_drive.FK_Calculate(left_wheel_angle, right_wheel_angle);
