/// \file
/// \brief circle node - publishes cmd_vel commands to cause the robot to drive
/// in a circle of a specified radius at a specified speed
///
...
///     stopped: whether it is stopped or not (when stopped - publish zero
///     twist)
/// PUBLISHES:
///     cmd_vel (geometry_msgs/Twist): publish cmd_vel commands
/// SERVICES:
///     control_service (nuturtle_control::control): control the angular
///     velocity and radius of the circle reverse_service (std_srvs::Empty):
///     reverse the trajectory of the robot stop_service (std_srvs::Empty): stop
///     the robot
...
bool control_callback(nuturtle_control::control::Request &req,
                      nuturtle_control::control::Response &res);
...
int main(int argc, char **argv) {
  ...
  ros::ServiceServer control_service =
      nh.advertiseService("control", control_callback);
  ros::ServiceServer reverse_service =
      nh.advertiseService("reverse", reverse_callback);
