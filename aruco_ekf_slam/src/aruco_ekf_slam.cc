...
ArUcoEKFSLAM::ArUcoEKFSLAM() : private_node("~") {
  ...
  image_sub =
      node_handle.subscribe(image_topic, 1, &ArUcoEKFSLAM::ImageCallback, this);
  encoder_sub = node_handle.subscribe(encoder_topic, 1,
                                      &ArUcoEKFSLAM::EncoderCallback, this);

  g_landmark_pub = node_handle.advertise<visualization_msgs::MarkerArray>(
      "ekf_slam/landmark", 1, this);
  g_robot_pose_pub =
      node_handle.advertise<geometry_msgs::PoseWithCovarianceStamped>(
          "ekf_slam/pose", 1, this);
...
void ArUcoEKFSLAM::GetParameter() {
  ...
  T_r_c_ << Trc[0], Trc[1], Trc[2], Trc[3], Trc[4], Trc[5], Trc[6], Trc[7],
      Trc[8], Trc[9], Trc[10], Trc[11], 0.0, 0.0, 0.0, 1.0;
}
...
void ArUcoEKFSLAM::addEncoder(const double &enl, const double &enr) {
  ...
  G_u_p << 0.5 * (cos(temp_angle) - delta_s * sin(temp_angle) / b_),
      0.5 * (cos(temp_angle) + delta_s * sin(temp_angle) / b_),
      0.5 * (sin(temp_angle) + delta_s * cos(temp_angle) / b_),
      0.5 * (sin(temp_angle) - delta_s * cos(temp_angle) / b_), 1.0 / b_,
      -1.0 / b_;
  Matrix<double, 3, 2> G_xi;
  G_xi << 1.0, 0.0, -delta_s * sin(temp_angle), 0.0, 1.0,
      delta_s * cos(temp_angle), 0.0, 0.0, 1.0;
  ...
  sigma_u << k_ * k_ * delta_s_r * delta_s_r, 0.0, 0.0,
      k_ * k_ * delta_s_l * delta_s_l;

  sigma_ = G_t * sigma_ * G_t.transpose() +
           F * G_u_p * sigma_u * G_u_p.transpose() * F.transpose();
...
void ArUcoEKFSLAM::addImage(const Mat &img) {
  ...
  for (Observation ob : obs) {
    Eigen::Matrix2d Q;
    Q << k_r_ * k_r_ * fabs(ob.r_ * ob.r_), 0.0, 0.0,
        k_phi_ * k_phi_ * fabs(ob.phi_ * ob.phi_);
    ...
    if (checkLandmark(ob.aruco_id_, i)) {
      ...
      MatrixXd H_v(2, 5);
      H_v << -sqrt_q * delta_x, -sqrt_q * delta_y, 0, sqrt_q * delta_x,
          sqrt_q * delta_y, delta_y, -delta_x, -q, -delta_y, delta_x;

      H_v = (1 / q) * H_v;

      MatrixXd Ht = H_v * F;

      MatrixXd K = sigma_ * Ht.transpose() *
                   (Ht * sigma_ * Ht.transpose() + Q).inverse();
    ...
    // else if the landmark doesn't exist
      ...
      G_p << 1, 0, -ob.r_ * sin(angle), 0, 1, ob.r_ * cos(angle);

      Matrix2d G_z;
      G_z << cos(angle), -ob.r_ * sin(angle), sin(angle), ob.r_ * cos(angle);
...
int ArUcoEKFSLAM::getObservations(const cv::Mat &img,
                                  std::vector<Observation> &obs) {
  std::vector<std::vector<cv::Point2f>> marker_corners;
  std::vector<int> IDs;
  std::vector<cv::Vec3d> rvs, tvs;
  cv::aruco::detectMarkers(img, dictionary_, marker_corners, IDs);
  cv::aruco::estimatePoseSingleMarkers(marker_corners, marker_length_, K_,
                                       dist_, rvs, tvs);
  ...
  for (size_t i = 0; i < IDs.size(); i++) {
    ...
    T_c_m << R.at<double>(0, 0), R.at<double>(0, 1), R.at<double>(0, 2),
        tvec[0], R.at<double>(1, 0), R.at<double>(1, 1), R.at<double>(1, 2),
        tvec[1], R.at<double>(2, 0), R.at<double>(2, 1), R.at<double>(2, 2),
        tvec[2], 0., 0., 0., 1.;
...
void ArUcoEKFSLAM::EncoderCallback(
    const geometry_msgs::QuaternionStamped::ConstPtr &encoder_ptr) {
...
visualization_msgs::MarkerArray ArUcoEKFSLAM::toRosMarker(double scale) {
  ...
  for (int i = 4; i < mu_.rows(); i += 2) {
    ...
    cv::Mat cvsigma_m = (cv::Mat_<double>(2, 2) << sigma_m(0, 0), sigma_m(0, 1),
                         sigma_m(1, 0), sigma_m(1, 1));
    cv::Mat eigen_value, eigen_vector;
    cv::eigen(cvsigma_m, eigen_value, eigen_vector);
    double angle =
        atan2(eigen_vector.at<double>(0, 1), eigen_vector.at<double>(0, 0));
